...
DockerdCheck=$(whereis docker |grep usr | cut -f 2 -d '/')
if [ "${DockerdCheck}" != "usr" ];then
		
        echo " 安装 $(docker -v) 完成"
fi

DockerComposedCheck=$(whereis docker-compose |grep usr | cut -f 2 -d '/')
if [ "${DockerComposedCheck}" != "usr" ];then
		sudo mv docker-compose-Linux-x86_64 /usr/local/bin/docker-compose 
        sudo chmod +x /usr/local/bin/docker-compose
        sudo ln -s /usr/local/bin/docker-compose /usr/bin/docker-compose
        echo " 安装 $(docker-compose -v) 完成"
fi


NPMdCheck=$(whereis npm |grep usr | cut -f 2 -d '/')
if [ "${NPMdCheck}" != "usr" ];then
		
        echo " 安装 $(npm -v)  $(node -v)  $(npx -v)完成"
fi
